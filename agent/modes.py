...
RESEARCH_SOURCE_TOOLS = frozenset({"web_search", "web_scrape"})
MAX_RESEARCH_SOURCES = 60


def _safe_source_url(value: Any) -> str:
    """Return a displayable public http(s) URL, or "" when it is unusable."""
...
def _source_key(url: str) -> str:
    """Group the same page reached through trivially different URLs."""
    try:
        parsed = urlsplit(url)
    except ValueError:
        return url.casefold()
    path = parsed.path.rstrip("/") or "/"
    return f"{_source_host(url)}{path}?{parsed.query}".casefold()
...
def _source_record(
    *,
    ...
    return {
        ...
        "title": " ".join(str(title or "").split())[:240] or _source_host(url) or url,
        "snippet": " ".join(str(snippet or "").split())[:400],
        "tool": tool,
        "query": _clean_query(query)[:240],
...
def extract_research_sources(
    ...
    """Return the citable sources a single web tool result actually produced.

    Only the identity of a source is kept (URL, title, short snippet). Result
    bodies stay in the transcript, so a citation list can never contradict the
    evidence the model reasoned over.
    """
    name = str(tool_name or "").strip()
    if name not in RESEARCH_SOURCE_TOOLS:
        return []
    if not isinstance(arguments, dict):
        arguments = {}
    try:
    ...
    if name == "web_scrape":
        if not isinstance(payload, dict) or payload.get("error"):
            return []
        url = _safe_source_url(payload.get("url") or arguments.get("url"))
        if not url:
            return []
        return [_source_record(
            url=url,
            title=payload.get("title"),
            snippet=payload.get("description") or payload.get("text"),
            tool=name,
    ...
    for entry in results:
        if not isinstance(entry, dict):
            continue
        url = _safe_source_url(entry.get("url") or entry.get("href"))
        if not url:
            continue
        page = entry.get("content")
        page = page if isinstance(page, dict) else {}
        sources.append(_source_record(
            url=url,
            title=entry.get("title") or page.get("title"),
            snippet=entry.get("snippet") or entry.get("body") or page.get("description"),
            tool=name,
            query=query,
            fetched=bool(page),
        ))
...
def merge_research_sources(
    ...
    *,
    ...
    """Append newly seen sources in discovery order, de-duplicated by page.

    ``collected`` is mutated in place and returned so callers can thread one
    citation list through a whole turn.
    """
    seen = {_source_key(str(source.get("url") or "")): source for source in collected}
    for source in new_sources:
        ...
        if existing is not None:
            # A later scrape of the same page carries better evidence than the
            # search snippet that first surfaced it.
            if source.get("fetched") and not existing.get("fetched"):
                existing["fetched"] = True
            if not existing.get("snippet") and source.get("snippet"):
                existing["snippet"] = source["snippet"]
            if source.get("title") and existing.get("title") in {"", existing.get("host"), url}:
...
def research_sources_summary(sources: list[dict[str, Any]]) -> str:
    """Short badge for a citation dropdown header, e.g. ``5 links · 3 sites``.

    The word "sources" is deliberately absent: every caller already labels the
    block itself, so repeating it reads as noise.
    """
    count = len(sources)
    if not count:
        return "no sources"
    hosts = len({str(source.get("host") or "") for source in sources if source.get("host")})
    label = f"{count} link{'' if count == 1 else 's'}"
    if hosts and hosts != count:
        return f"{label} · {hosts} site{'' if hosts == 1 else 's'}"
    return label
