...
- **Deep Research sources dropdown** — a collapsed **Sources** panel sits under the answer in both Web and TUI, listing every page the research pass actually reached: title, link, host, the search query that surfaced it, whether the page itself was read, and a short snippet. Only public `http(s)` results are listed, duplicates across search rounds collapse to one entry, and the list is stored with the answer so it survives a reload. The classic CLI prints the same list beneath the response.
